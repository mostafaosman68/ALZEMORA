...
class RealtimeVoiceIdentifier:
    ...
    def start_realtime_identification(self, buffer_seconds=5, update_interval=3.0):
        ...
        print(f"Analyzing voice every {update_interval} seconds.")
        ...
        with sd.InputStream(samplerate=self.sample_rate, channels=1, 
            ...
            while self.is_recording:
                ...
                if current_time - last_process_time >= update_interval:
                    ...
                    print("\n" + "="*40)
                    print(f"Analyzing voice at {time.strftime('%H:%M:%S')}")
                    ...
                    speaker, confidence = self.identify_speaker(audio_data=normalized_buffer)
...
if __name__ == "__main__":
    ...
    identifier = RealtimeVoiceIdentifier(
        samples_dir=r"C:/Users/ITD/Desktop/GUI/Data", 
        threshold=0.45  # Using a lower threshold based on previous results
    )
    
    try:
        # Start real-time identification with 3-second intervals and a 5-second buffer
        identifier.start_realtime_identification(buffer_seconds=5, update_interval=3.0)
